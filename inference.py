...
infile = open('classification.model', 'rb')
...
def predict_tweet():
    ...
    def clean_text(text):        
        stop_words = stopwords.words('english') + ['u', 'ur', '4',   
                     '2', 'im', 'dont', 'doin', 'ure']        
        text = text.lower()        
        text = re.sub('\[.*?\]', '', text)        
    ...
    if pred >= 50:        
        return render_template('index.html', exp=exp, 
               prediction=f'Emergency; confidence ({pred}%)')    
    else:        
        return render_template('index.html', exp=exp, 
               prediction=f'Non-emergency; confidence ({100-pred}%)')
